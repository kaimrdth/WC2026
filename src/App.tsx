...
import { RotateCcw, ChevronDown, ChevronUp, Info, Star, Check, X, Loader2, AlertCircle, Users, BarChart3, Medal, Ticket, ArrowDown, ArrowUp, ArrowLeftRight, CalendarClock, Sparkles, Trophy, Plus, Minus, Crown } from "lucide-react";
...
const KO_FIXTURES: KoFixture[] = [
  ...
  { id:"ko-R16-2", round:"R16", n:2, kickoff:"2026-07-04T21:00Z", venue:"Lincoln Financial Field", city:"Philadelphia, Pennsylvania", homeSlot:"Round of 32 2 Winner", awaySlot:"Round of 32 3 Winner" },
  { id:"ko-R16-3", round:"R16", n:3, kickoff:"2026-07-05T20:00Z", venue:"MetLife Stadium", city:"East Rutherford, New Jersey", homeSlot:"Round of 32 5 Winner", awaySlot:"Round of 32 6 Winner" },
  ...
  { id:"ko-R16-5", round:"R16", n:5, kickoff:"2026-07-06T19:00Z", venue:"AT&T Stadium", city:"Arlington, Texas", homeSlot:"Round of 32 11 Winner", awaySlot:"Round of 32 16 Winner" },
  ...
  { id:"ko-R16-7", round:"R16", n:7, kickoff:"2026-07-07T16:00Z", venue:"Mercedes-Benz Stadium", city:"Atlanta, Georgia", homeSlot:"Round of 32 14 Winner", awaySlot:"Round of 32 12 Winner" },
...
const BR_DETAIL=["flags","mini","full"] as const;
const BR_DETAIL_LABEL=["Flags","Compact","Full"];

// The whole bracket always fits the viewport (object-fit: contain) and stays centred — no
// panning. The −/＋ control changes the level of detail instead of zooming: less detail makes
// the cards more compact, so the tree scales up bigger and clearer; more detail packs in
// dates/venues at a smaller overall scale.
function KoBracket({ko,koResults,liveByFixture,detailIds,onOpenDetail,onSelectTeam,onPreviewKo}:{
  ...
  const measure=useCallback(()=>{
    const vp=viewportRef.current, c=contentRef.current; if(!vp||!c) return;
    const nw=c.offsetWidth, nh=c.offsetHeight;
    const vw=vp.clientWidth, vh=vp.clientHeight;
    if(!nw||!nh) return;
    const scale=Math.min(vw/nw, vh/nh, 1.4);
    setFit({scale, x:(vw-nw*scale)/2, y:(vh-nh*scale)/2});
  },[]);

  // Re-measure after a detail change (card sizes change → natural size changes) and on resize.
  useLayoutEffect(()=>{ measure(); },[detail,ko.length,measure]);
  useEffect(()=>{
      ...
      <div className="wc-br2-viewport" ref={viewportRef}>
        ...
        <div className="wc-br2-zoom" role="group" aria-label="Bracket detail level">
          <button type="button" className="wc-br2-zbtn" aria-label="Less detail" disabled={detail===0}
            onClick={()=>setDetail(d=>Math.max(0,d-1))}><Minus size={15}/></button>
          <span className="wc-br2-zpct">{BR_DETAIL_LABEL[detail]}</span>
          <button type="button" className="wc-br2-zbtn" aria-label="More detail" disabled={detail===2}
...
// ── Stage review (group stage, later knockout rounds) ─────────────────────────
const groupStageComplete=(r:Record<string,ScoreResult>)=>GROUP_LETTERS.every(l=>groupIsComplete(l,r));

interface StageScorer{player:string;team:string;goals:number;}
function stageTopScorers(goals:GoalEvent[],n:number):StageScorer[]{
  const m=new Map<string,StageScorer>();
  for(const g of goals){ if(g.kind==="owngoal")continue; const k=g.player+"|"+g.teamCode; const e=m.get(k)??{player:g.player,team:g.teamCode,goals:0}; e.goals++; m.set(k,e); }
  return [...m.values()].sort((a,b)=>b.goals-a.goals||a.player.localeCompare(b.player)).slice(0,n);
}
interface StageMatch{fid:string;home:string;away:string;hg:number;ag:number;margin:number;total:number;}
function stageMatches(groupResults:Record<string,ScoreResult>):StageMatch[]{
  const out:StageMatch[]=[];
  for(const [fid,r] of Object.entries(groupResults)){
    const f=FIXTURE_BY_ID[fid]; if(!f) continue;
    out.push({fid,home:f.homeCode,away:f.awayCode,hg:r.homeGoals,ag:r.awayGoals,margin:Math.abs(r.homeGoals-r.awayGoals),total:r.homeGoals+r.awayGoals});
  }
  return out;
}
// Tournament-wide team stat totals from match boxscores (shared by the review + stats page).
function aggTeamStats(matchStats:Record<string,EventStat>):Map<string,Record<string,number>>{
  const m=new Map<string,Record<string,number>>();
  const add=(c:string,v:Record<string,number>)=>{ const e=m.get(c)??{matches:0}; e.matches=(e.matches??0)+1; for(const k of AGG_STAT_KEYS)e[k]=(e[k]??0)+(v[k]??0); m.set(c,e); };
  for(const ev of Object.values(matchStats)){ add(ev.home.code,ev.home.v); add(ev.away.code,ev.away.v); }
  return m;
}
function buildGroupStageFacts(groupResults:Record<string,ScoreResult>, qualifiers:ReturnType<typeof computeQualifiers>, goals:GoalEvent[], cards:CardEvent[], matchStats:Record<string,EventStat>):string{
  const nm=(c:string)=>TEAM_BY_CODE[c]?.name??c;
  const qCodes=new Set([...qualifiers.winners,...qualifiers.runnersUp,...qualifiers.bestThirds].map(t=>t.code));
  const fallen=TEAMS.filter(t=>!qCodes.has(t.code)).sort((a,b)=>a.fifaRank-b.fifaRank).slice(0,4);
  const risen=[...qualifiers.bestThirds,...qualifiers.runnersUp].sort((a,b)=>b.fifaRank-a.fifaRank).slice(0,4);
  const scorers=stageTopScorers(goals,5);
  const ms=stageMatches(groupResults);
  const big=[...ms].sort((a,b)=>b.margin-a.margin||b.total-a.total)[0];
  const fest=[...ms].sort((a,b)=>b.total-a.total||b.margin-a.margin)[0];
  const agg=aggTeamStats(matchStats);
  ...
  const tline=(t:Team|StandingRow)=>`${t.name} (#${t.fifaRank})`;
  return [
    "TYPE: group-stage retrospective",
    "Stage: the World Cup 2026 group stage is complete; all 12 groups finished and 32 teams reached the Round of 32.",
    `Group winners: ${qualifiers.winners.map(t=>`${t.name} (Grp ${t.origin})`).join(", ")}`,
    `Runners-up: ${qualifiers.runnersUp.map(t=>`${t.name} (Grp ${t.origin})`).join(", ")}`,
    `Best third-place qualifiers: ${qualifiers.bestThirds.map(t=>`${t.name} (Grp ${t.origin})`).join(", ")}`,
    `Big names eliminated (lower number = higher FIFA rank): ${fallen.map(tline).join(", ")||"none"}`,
    `Lower-ranked sides who advanced: ${risen.map(tline).join(", ")||"none"}`,
    `Top scorers: ${scorers.map(s=>`${s.player} (${nm(s.team)}) ${s.goals}`).join(", ")||"n/a"}`,
    big?`Biggest win: ${nm(big.home)} ${big.hg}-${big.ag} ${nm(big.away)}`:"",
    fest?`Highest-scoring match: ${nm(fest.home)} ${fest.hg}-${fest.ag} ${nm(fest.away)} (${fest.total} goals)`:"",
    `Red cards in the group stage: ${cards.filter(c=>c.red).length}`,
    poss?`Most possession: ${nm(poss.c)} (~${Math.round(poss.v)}% avg)`:"",
    shots?`Most shots: ${nm(shots.c)} (${shots.v})`:"",
    "Guidance: tell the story of the group stage — the drama, the biggest shocks, the standout performers, and which teams look dangerous heading into the knockouts. 2-3 short paragraphs, accurate to the facts.",
  ].filter(Boolean).join("\n");
}
function fallbackGroupStageNarrative(qualifiers:ReturnType<typeof computeQualifiers>, goals:GoalEvent[]):string{
  const topW=qualifiers.winners[0];
  const boot=stageTopScorers(goals,1)[0];
  const qCodes=new Set([...qualifiers.winners,...qualifiers.runnersUp,...qualifiers.bestThirds].map(t=>t.code));
  const fallen=TEAMS.filter(t=>!qCodes.has(t.code)).sort((a,b)=>a.fifaRank-b.fifaRank)[0];
  const p:string[]=["The group stage is done and the Round of 32 is set."];
  if(topW) p.push(`${topW.name} were among the standout group winners.`);
  if(fallen) p.push(`The biggest casualty was ${fallen.name}, out despite arriving ranked #${fallen.fifaRank} in the world.`);
  if(boot) p.push(`${boot.player} leads the Golden Boot race on ${boot.goals}.`);
  p.push("The knockouts are next.");
  return p.join(" ");
}
...
function StageNarrative({facts,cacheKey,fallback,label,onSelectTeam,onSelectGroup}:{
  ...
  useEffect(()=>{
    let alive=true;
    const lk=`wc-stage:${cacheKey}`;
    try{ const c=localStorage.getItem(lk); if(c){ setText(c); setStatus("ok"); return; } }catch{ /* ignore */ }
    setText(fallback); setStatus("loading");
    fetch(STAGE_DIGEST_ENDPOINT,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({facts,cacheKey})})
      .then(async r=>r.ok?await r.json():Promise.reject(new Error(`HTTP ${r.status}`)))
      .then(j=>{ const t=String(j?.text||"").trim(); if(!alive||!t)return; setText(t); setStatus("ok"); try{ localStorage.setItem(lk,t); }catch{ /* ignore */ } })
...
function GroupStageReview({groupResults,qualifiers,goals,cards,matchStats,detailIds,onOpenDetail,onSelectTeam,onSelectGroup}:{
  ...
  const facts=useMemo(()=>buildGroupStageFacts(groupResults,qualifiers,goals,cards,matchStats),[groupResults,qualifiers,goals,cards,matchStats]);
  const fallback=useMemo(()=>fallbackGroupStageNarrative(qualifiers,goals),[qualifiers,goals]);
  const cacheKey=useMemo(()=>`groupstage:${digestCacheKey(facts)}`,[facts]);

  const scorers=useMemo(()=>stageTopScorers(goals,6),[goals]);
  ...
  const agg=useMemo(()=>aggTeamStats(matchStats),[matchStats]);
  ...
  if(!complete){
    return (
      <div className="wc-stats-empty">
        ...
        <p>The Group Stage in Review unlocks once all 12 groups have finished.</p>
      </div>
    );
  }
  ...
  const statRow=(label:string, lead:{c:string;v:number}|undefined, fmt:(v:number)=>string)=> lead?(
    <div className="wc-rv-statrow">
      <span className="wc-rv-statlabel">{label}</span>
    ...
    <div className="wc-review">
      ...
      <div className="wc-review-grid">
        <section className="wc-stats-panel">
          <h3 className="wc-stats-h"><Medal size={15}/> Golden Boot race</h3>
...
function StatsView({goals,cards,matchesPlayed,matchStats,onSelectTeam}:{
  ...
  const teamAgg=useMemo(()=>aggTeamStats(matchStats),[matchStats]);
...
const STAGE_DIGEST_ENDPOINT = "/.netlify/functions/stage-digest";
...
export default function App() {
  ...
  const groupsComplete=useMemo(()=>groupStageComplete(groupResults),[groupResults]);
...
@property --pitch{syntax:"<color>";inherits:true;initial-value:#EEF1F6;}
@property --pitch-deep{syntax:"<color>";inherits:true;initial-value:#E2E7EF;}
@property --pitch-card{syntax:"<color>";inherits:true;initial-value:#FFFFFF;}
@property --pitch-line{syntax:"<color>";inherits:true;initial-value:rgba(24,32,47,.13);}
@property --gold{syntax:"<color>";inherits:true;initial-value:#A8780F;}
@property --gold-soft{syntax:"<color>";inherits:true;initial-value:rgba(168,120,15,.14);}
@property --chalk{syntax:"<color>";inherits:true;initial-value:#1B2230;}
@property --chalk-dim{syntax:"<color>";inherits:true;initial-value:#5A6473;}
.wc-app{
  --pitch:#EEF1F6;--pitch-deep:#E2E7EF;--pitch-card:#FFFFFF;--pitch-line:rgba(24,32,47,.13);
  --chalk:#1B2230;--chalk-dim:#5A6473;--gold:#A8780F;--gold-soft:rgba(168,120,15,.14);
  --green-win:#1E6B43;--red-loss:rgba(220,53,69,.10);--live:#E23B42;
  font-family:'Space Grotesk',sans-serif;color:var(--chalk);
  /* Clean off-white base with a faint red/green/blue ambient — the Trionda host colours
     washing in at the edges instead of the old navy. */
  background:
    radial-gradient(72% 55% at 8% -6%, rgba(77,139,255,.13), transparent 60%),
    radial-gradient(62% 50% at 94% 3%, rgba(255,90,90,.11), transparent 60%),
    radial-gradient(78% 62% at 50% 110%, rgba(46,192,106,.11), transparent 58%),
    var(--pitch);
...
@media(prefers-reduced-motion:reduce){.wc-hero-title-btn{animation:none;background:none;-webkit-text-fill-color:var(--chalk);color:var(--chalk);}}
...
.wc-live-error{display:inline-flex;align-items:center;margin-top:.7rem;color:var(--live);font-size:.76rem;font-weight:600;}
...
.wc-nav-teams{border-style:dashed;border-color:var(--pitch-line);}
...
.wc-row-me{outline:1px solid var(--gold);outline-offset:-1px;font-weight:700;}
...
.wc-standings-livetag{display:inline-flex;align-items:center;gap:.28rem;margin-left:.4rem;font-size:.56rem;font-weight:800;letter-spacing:.05em;color:var(--live);vertical-align:middle;}
...
.wc-row-live .wc-pts{color:var(--live);}
...
.wc-team-ko-tag-out{background:rgba(220,53,69,.14);color:var(--live);}
...
.wc-team-ko-res.loss{color:var(--live);border-color:rgba(220,53,69,.35);}
.wc-team-ko-res.live{color:var(--live);border-color:rgba(255,77,77,.45);}
...
.wc-digest-err{color:var(--live);font-size:.78rem;}
...
.wc-micro-team{background:var(--pitch-deep);border-color:var(--pitch-line);}
...
.wc-livebar-tag{display:inline-flex;align-items:center;gap:.4rem;font-size:.66rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--live);flex-shrink:0;}
...
.wc-livebar-clock{font-family:'JetBrains Mono',monospace;font-size:.62rem;font-weight:700;color:var(--live);}
...
.wc-match-livetag{position:absolute;top:.45rem;right:.7rem;display:inline-flex;align-items:center;gap:.3rem;font-size:.56rem;font-weight:800;letter-spacing:.05em;color:var(--live);z-index:1;}
.wc-match-score-live{color:var(--live);}

/* schedule live row */
.wc-sched-row-live{border-color:rgba(255,77,77,.5);animation:wc-liveglow 1.8s ease-in-out infinite;}
.wc-sched-score-live{color:var(--live);}
.wc-sched-livetag{display:inline-flex;align-items:center;gap:.3rem;font-weight:800;color:var(--live);}
...
.wc-hub-status-out{color:var(--live);background:var(--red-loss);border:1px solid rgba(220,53,69,.35);}
...
.wc-hub-result-L .wc-hub-outcome{color:var(--live);}
...
.wc-thirds-live-tag{color:var(--live);font-weight:800;font-size:.72rem;letter-spacing:.02em;}
.wc-thirds-live-dot{display:inline-flex;align-items:center;margin-left:.4rem;padding:.05rem .3rem;border-radius:.25rem;font-size:.52rem;font-weight:800;letter-spacing:.05em;color:var(--live);background:rgba(255,77,77,.14);vertical-align:middle;animation:wc-pulse 1.6s ease-out infinite;}
...
.wc-br2-viewport{position:relative;overflow:hidden;height:min(74vh,680px);min-height:320px;border:1px solid var(--pitch-line);border-radius:16px;background:radial-gradient(120% 80% at 50% 0%,rgba(77,139,255,.07),transparent 60%),radial-gradient(120% 80% at 50% 100%,rgba(46,192,106,.06),transparent 62%),var(--pitch-deep);}
.wc-br2{display:flex;width:max-content;align-items:stretch;padding:.5rem .8rem;--g:.7rem;--ln:rgba(27,34,48,.24);}
...
/* level-of-detail: full = everything; compact drops the footer; flags shows the two flags
   side-by-side in shorter rows, so with less detail the whole tree scales up bigger + clearer. */
.wc-br2[data-lod="mini"] .wc-br2-foot{display:none;}
.wc-br2[data-lod="mini"] .wc-br2-match{min-height:40px;}
.wc-br2[data-lod="flags"] .wc-br2-foot,.wc-br2[data-lod="flags"] .wc-br2-name,.wc-br2[data-lod="flags"] .wc-br2-score{display:none;}
.wc-br2[data-lod="flags"] .wc-br2-col{width:72px;}
.wc-br2[data-lod="flags"] .wc-br2-match{min-height:38px;}
.wc-br2[data-lod="flags"] .wc-br2-card{align-items:center;padding:.2rem;}
...
.wc-br2-live{display:inline-flex;align-items:center;gap:.25rem;color:var(--live);font-weight:700;}
...
.wc-recap-round-active{color:var(--pitch-card);background:var(--chalk);border-color:var(--chalk);}
...
.wc-recap-round:not(.wc-recap-round-active) .wc-recap-prog{background:rgba(0,0,0,.07);}
...
.wc-stage-narrative p{margin:.55rem 0 0;font-size:.9rem;line-height:1.65;color:var(--chalk);}
.wc-review-grid{display:grid;grid-template-columns:1fr;gap:1rem;}
...
.wc-shock-out-head{color:var(--live);margin-bottom:.2rem;}
...
.wc-shock-row:hover{background:rgba(0,0,0,.045);}
...
.wc-rv-statval{margin-left:auto;font-family:'JetBrains Mono',monospace;font-weight:700;color:var(--chalk);}
...
.wc-pb-off{color:var(--live);display:inline-flex;}
...
.wc-detail-live-tag{display:inline-flex;align-items:center;gap:.3rem;font-size:.58rem;font-weight:800;letter-spacing:.05em;color:var(--live);}
.wc-detail-score-num-live{color:var(--live);}
