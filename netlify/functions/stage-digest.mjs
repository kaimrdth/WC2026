// Serverless proxy for stage retrospectives (group stage, knockout rounds).
// Facts are built client-side from tournament state; this turns that compact payload
// into a short multi-paragraph review and shares the cached result across users.

import { getStore } from "@netlify/blobs";

const MODELS = ["gemini-2.5-flash", "gemini-2.5-flash-lite"];
const MEM_TTL = 6 * 60 * 60 * 1000; // a completed stage doesn't change — cache hard
const mem = new Map();
const inflight = new Map();

const SYSTEM = `You write a punchy FIFA World Cup 2026 stage retrospective inside a live scores app.
Use ONLY the supplied facts — never invent scores, scorers, injuries, transfers, or quotes.
Tell the story of the stage: the headline drama, the biggest shocks (favourites who fell, underdogs who rose),
standout performers, and which sides look dangerous heading on. Interpret, don't just list — but stay accurate.
Write 2 to 3 short paragraphs separated by a blank line. Plain text only: no markdown, no headers, no emojis, no bullet lists.
Sound like a sharp, calm football writer. If facts are thin, stay brief.`;
...
function blobStore() {
  try { return getStore("stage-digest"); } catch { return null; }
}
...
async function callModel(model, key, facts) {
  const generationConfig = { temperature: 0.55, maxOutputTokens: 520 };
